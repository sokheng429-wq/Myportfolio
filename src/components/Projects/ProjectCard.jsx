...
function ProjectCard({title , description , imgSrc , navId ,codeLink , codeLinks, demoLink, demoLinks, technologies , className}) {
    
    const [showLightbox, setShowLightbox] = useState(false);

    const handleShow = () => setShowLightbox(true);
    const handleClose = () => setShowLightbox(false);
    const projectCodeLinks = codeLinks ?? (codeLink ? [{ label: 'Code', url: codeLink }] : []);
    const projectDemoLinks = demoLinks ?? (demoLink ? [{ label: 'Demo', url: demoLink }] : []);
         ...
         <div className="card off-white mb-8 pt-3" style={{ maxWidth: "100%" }}>
                    ...
                    <div className={`row g-0 ${className}`}>
                        ...
                        <div className="card-body text-center ">
                            ...
                            {projectDemoLinks.map(({ label, url }) => (
                            <a key={url} href={url} target="_blank" rel="noreferrer" className="text-decoration-none">
